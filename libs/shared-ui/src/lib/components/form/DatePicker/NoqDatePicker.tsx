...
import { useClickOutside, useControlledState } from "../../../useHooks";
...
export function NoqDatePicker({
    ...
    useEffect(() => {
        if (
            ...
            && isValidDate(`${inputValue.year}-${inputValue.month}-${inputValue.day}`)
        ) {
    ...
    // 超出范围给与错误状态
    useEffect(() => {
        const lessThanMinValue = minValue && valueState && valueState.compare(minValue) < 0;
        const greaterThanMaxValue = maxValue && valueState && valueState.compare(maxValue) > 0;
        if (lessThanMinValue || greaterThanMaxValue) {
            setError(true);
        } else {
            setError(false);
        }
    }, [minValue, valueState, maxValue])
            ...
            <FrankPopoverGeneral
                popoverTrigger={
                    <div
                        ...
                        className={clsx("bg-[#FFF] rounded-md pl-2 pr-[6px] py-[6px] flex items-center justify-between select-none border-1", {
                            " border-[#B5B5B5]": !error,
                            "border-[#F31260] text-[#F31260]": error
                        })}
                    >
                        {/* date input */}
                        <div
                        ...
                        >
                            {/* Year Input */}
                            <input
                                className={clsx("w-[35%] h-full focus:outline-none font-inter text-[13px] focus:bg-gray-100 rounded-md caret-transparent text-center cursor-pointer", {
                                    "text-[#303030]": !error,
                                    "text-[#F31260]": error
                                })}
                                type='text'
                                ...
                                onChange={(e) => {
                                    ...
                                    if (shouldClearOnInput.year && lastInputNumber && lastInputNumber !== 0 && !isNaN(lastInputNumber)) {
                                    ...
                                    if (!shouldClearOnInput.year && lastInputNumber !== null && !isNaN(lastInputNumber)) {
                            ...
                            {/* Month Input */}
                            <input
                                className={clsx("w-[22%] h-full focus:outline-none font-inter text-[13px] focus:bg-gray-100 rounded-md caret-transparent text-center cursor-pointer", {
                                    "text-[#303030]": !error,
                                    "text-[#F31260]": error
                                })}
                                type='text'
                                ...
                                value={inputValue.month ? formatNumberString(inputValue.month, 2) : inputValue.month}
                                onChange={(e) => {
                                    const inputEvent = e.nativeEvent as InputEvent;
                                    const lastInputNumber = inputEvent.data ? +inputEvent.data : null
                                    let newMonthValue = inputValue.month;
                                    if (shouldClearOnInput.month && lastInputNumber !== null && !isNaN(lastInputNumber)) {
                            ...
                            {/* Day Input */}
                            <input
                                className={clsx("w-[20%] h-full focus:outline-none font-inter text-[13px] focus:bg-gray-100 rounded-md caret-transparent text-center cursor-pointer", {
                                    "text-[#303030]": !error,
                                    "text-[#F31260]": error
                                })}
                                type='text'
                                ...
                                value={inputValue.day ? formatNumberString(inputValue.day, 2) : inputValue.day}
                                onChange={(e) => {
                                    const inputEvent = e.nativeEvent as InputEvent;
                                    const lastInputNumber = inputEvent.data ? +inputEvent.data : null
                                    let newDayValue = inputValue.day;
                                    if (shouldClearOnInput.day && lastInputNumber !== null && !isNaN(lastInputNumber)) {
                        ...
                        {/* end content */}
                        <div
                            className="cursor-pointer text-[#4A4A4A]"
                            onClick={() => {
                                setIsOpen(prev => !prev);
                            }}
                        >
